...
json="$(curl -fsSL --show-error --connect-timeout 8 --max-time 25 --retry 2 "https://api.github.com/repos/$repo/releases?per_page=$limit" 2>/tmp/omnihive-releases.err)" || {
	msg="$(cat /tmp/omnihive-releases.err 2>/dev/null || true)"
