...
#
# Recovery steps:
#   a. Check QMI health via OmniHive API (skip if service not running)
...
#   e. Restart OmniHive service (if running)
#   f. Verify QMI health
#
...
# Check QMI health via OmniHive API
check_qmi_health() {
	local username password port token device_running

	/etc/init.d/omnihive running >/dev/null 2>&1 || return 1
...
if ! ls /dev/cdc-wdm* >/dev/null 2>&1; then
	...
	/usr/share/omnihive/driver_bind.sh bind_qmi "$qmi_iface" "$qmi_vid" "$qmi_pid" >/dev/null 2>&1 \
		|| result_fail "QMI 接口绑定失败 ($qmi_iface)"
	sleep 2
fi

# Step 5 — restart OmniHive so it picks up the fresh cdc-wdm fd
if /etc/init.d/omnihive running >/dev/null 2>&1; then
	/etc/init.d/omnihive restart 2>/dev/null || true
	sleep 10
fi
