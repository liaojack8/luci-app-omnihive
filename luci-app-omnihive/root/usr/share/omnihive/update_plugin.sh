...
json="$(curl -fsSL --show-error --connect-timeout 15 --retry 2 "https://api.github.com/repos/$PLUGIN_REPO/releases/latest" 2>/tmp/omnihive-plugin-update.err)" || {
	msg="$(cat /tmp/omnihive-plugin-update.err 2>/dev/null || true)"
	fail "查询插件最新版本失败: $msg"
}
