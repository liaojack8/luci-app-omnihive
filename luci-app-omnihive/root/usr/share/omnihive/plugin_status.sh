...
VERSION_FILE="/usr/share/omnihive/plugin_version"
limit="${1:-5}"
...
current="$(pkg_installed_version luci-app-omnihive 2>/dev/null || true)"
[ -n "$current" ] || current="$(cat "$VERSION_FILE" 2>/dev/null || true)"
[ -n "$current" ] || current="unknown"
current_norm="$(normalize_plugin_version "$current")"

json="$(curl -fsSL --show-error --connect-timeout 8 --max-time 25 --retry 2 "https://api.github.com/repos/$PLUGIN_REPO/releases?per_page=$limit" 2>/tmp/omnihive-plugin-releases.err)" || {
	msg="$(cat /tmp/omnihive-plugin-releases.err 2>/dev/null || true)"
