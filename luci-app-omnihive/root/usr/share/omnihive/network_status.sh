#!/bin/sh
# network_status.sh — Query OmniHive network integration status
#
# Outputs JSON for the LuCI "网络管理" tab, combining:
#   - OmniHive API device status (data_connected, interface, IP, signal)
#   - OpenWrt netifd config (network.omnihive exists? in wan zone?)
...
if [ -n "$api_data" ]; then
	...
	omnihive_iface="$(api_field interface)"
...
if uci -q get network.omnihive >/dev/null 2>&1; then
	...
	netifd_device="$(uci -q get network.omnihive.device 2>/dev/null || true)"
fi
...
# ---------------------------------------------------------------------------
# Use the interface name from OmniHive API, or fall back to scanning wwan*
wwan_iface="$omnihive_iface"
wwan_operstate=""
wwan_carrier=""
wwan_ipv4=""
wwan_ipv6=""
...
# ---------------------------------------------------------------------------
printf '{'
printf '"omnihive_running":%s,' "$(/etc/init.d/omnihive running >/dev/null 2>&1 && printf true || printf false)"
...
printf '"interface":"%s",' "$(json_escape "$omnihive_iface")"
