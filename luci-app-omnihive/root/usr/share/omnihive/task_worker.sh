...
. /usr/share/omnihive/task_lib.sh
...
post_install_qmi_check() {
	...
	recover_output="$(/usr/share/omnihive/recover_qmi.sh 2>&1)" || true
...
install_core() {
	...
	uci set omnihive.main.release_repo="https://github.com/$repo" 2>/dev/null || true
	uci set omnihive.main.version="$selected_version" 2>/dev/null || true
	[ -z "$core_arch" ] || uci set omnihive.main.core_arch="$core_arch" 2>/dev/null || true
	uci commit omnihive 2>/dev/null || true

	task_log "$id" "查询 OmniHive Release"
	task_write_status "$id" "$type" "running" "prepare" "正在查询 OmniHive Release" "" 0 0 0 0
	...
	if [ "$was_running" = "1" ]; then
		task_write_status "$id" "$type" "running" "restart" "正在重启 OmniHive 服务" "" 0 0 0 0
	...
	finish_ok "已安装 OmniHive 核心 $version"
}
...
update_plugin() {
	...
	install_log="/tmp/omnihive-plugin-install.log"
	ext="$(pkg_ext)"
	...
	asset="$(select_plugin_asset "$json")" || fail "最新 Release 未找到 luci-app-omnihive *.${ext} 安装包"
	...
	installed_version="$(pkg_installed_version luci-app-omnihive)"
	installed_norm="$(normalize_plugin_version "${installed_version:-}")"
	[ -n "$installed_norm" ] || installed_norm="$(cat /usr/share/omnihive/plugin_version 2>/dev/null || true)"
	[ "$installed_norm" = "$tag_norm" ] || {
		msg="$(tail -n 20 "$install_log" 2>/dev/null || true)"
		fail "安装后版本仍为 ${installed_version:-unknown}，期望 $tag。$msg"
	}

	printf '%s\n' "$tag_norm" > /usr/share/omnihive/plugin_version 2>/dev/null || true
...
convert_identity() {
	...
	if result="$(OMNIHIVE_TASK_ID="$id" OMNIHIVE_TASK_TYPE="$type" /usr/share/omnihive/device_tools.sh convert "$port" "$target" 2>&1)"; then
...
switch_usbnet() {
	...
	if result="$(OMNIHIVE_TASK_ID="$id" OMNIHIVE_TASK_TYPE="$type" /usr/share/omnihive/device_tools.sh switch_usbnet "$port" "$target" 2>&1)"; then
		message="$(printf '%s' "$result" | jsonfilter -e '@.message' 2>/dev/null || true)"
...
probe_device() {
	local tmp="/tmp/omnihive/device-probe.json.tmp"
	local cache="/tmp/omnihive/device-probe.json"
	local result message

	task_write_status "$id" "$type" "running" "probe" "正在探测 USB 串口设备" "" 0 0 0 0
	if result="$(OMNIHIVE_TASK_ID="$id" OMNIHIVE_TASK_TYPE="$type" /usr/share/omnihive/device_tools.sh probe 2>&1)"; then
		mkdir -p /tmp/omnihive
		printf '%s\n' "$result" > "$tmp"
		mv -f "$tmp" "$cache"
		finish_ok "设备探测完成"
	else
...
upload_core() {
	local upload_file="$DOWNLOAD_DIR/omnihive-core-upload"
	local version="--"
	...
	task_write_status "$id" "$type" "running" "restart" "正在启动 OmniHive 服务" "" 0 0 0 0
	if ! /etc/init.d/omnihive start >/tmp/omnihive-start.log 2>&1; then
		fail "核心已安装但服务启动失败"
	fi
	...
	post_install_qmi_check "$was_running"
	rm -f "$upload_file" "$BIN_DIR/omnihive.bak"
	finish_ok "已安装 OmniHive 核心（$version）"
}
