...
task_enabled() {
	[ -n "${OMNIHIVE_TASK_ID:-}" ] && [ -n "${OMNIHIVE_TASK_TYPE:-}" ] && [ -f /usr/share/omnihive/task_lib.sh ]
}
...
task_progress() {
	...
	if task_enabled; then
		# shellcheck source=/usr/share/omnihive/task_lib.sh
		. /usr/share/omnihive/task_lib.sh
		task_log "$OMNIHIVE_TASK_ID" "$message"
		task_write_status "$OMNIHIVE_TASK_ID" "$OMNIHIVE_TASK_TYPE" "running" "$stage" "$message" "" 0 0 0 0
	fi
}
...
stop_omnihive() {
	task_progress service "停止 OmniHive 服务"
	/etc/init.d/omnihive stop 2>&1 || true
}

start_omnihive() {
	task_progress service "启动 OmniHive 服务"
	/etc/init.d/omnihive start 2>&1 || true
}
...
convert_json() {
	...
	output="停止 OmniHive 服务...\n"
	output="$output$(stop_omnihive)\n\n"
	...
	output="${output}启动 OmniHive 服务...\n"
	output="$output$(start_omnihive)\n"
	...
	case "$ok" in
		...
		partial)
			...
			;;
		*)
			printf '{"ok":false,"message":"写入命令未确认成功，OmniHive 已启动","output":"%s"}\n' "$(json_escape "$output")"
			exit 1
			;;
	esac
}
