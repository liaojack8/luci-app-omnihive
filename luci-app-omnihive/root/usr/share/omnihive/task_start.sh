...
case "$type" in
	install_core|upload_core|update_plugin|convert_identity|switch_usbnet|probe_device) ;;
...
/usr/share/omnihive/task_worker.sh "$id" "$type" "$@" >/tmp/omnihive/task-$id.out 2>&1 &
pid="$!"
