...
pkg_install_file() {
	local file="$1"
	local log="${2:-/tmp/omnihive-pkg-install.log}"
...
# From a GitHub release assets JSON blob, pick luci-app-omnihive package for current pkg mgr.
...
select_plugin_asset() {
	...
	while :; do
		...
		case "$ext" in
			ipk)
				case "$name" in
					luci-app-omnihive_*_all.ipk)
						preferred="$name"
						break
						;;
					luci-app-omnihive_*_*.ipk|luci-app-omnihive_*.ipk)
						[ -n "$any" ] || any="$name"
						;;
				esac
				;;
			apk)
				case "$name" in
					luci-app-omnihive_*_all.apk|luci-app-omnihive-*-r*.apk)
						preferred="$name"
						break
						;;
					luci-app-omnihive*.apk)
						[ -n "$any" ] || any="$name"
						;;
				esac
				;;
