...
BIN="/etc/omnihive/bin/omnihive"
VERSION_FILE="/etc/omnihive/bin/version"
ARCH_FILE="/etc/omnihive/bin/arch"
METRICS_STATE="/tmp/omnihive/status.metrics"
...
uci_get() {
	...
	value="$(uci -q get "omnihive.main.$key" 2>/dev/null || true)"
...
omnihive_pids() {
	...
	for pid in /proc/[0-9]*; do
		...
		case "$cmd" in
			*"/etc/omnihive/bin/omnihive"*) printf '%s\n' "$pid" ;;
		esac
	done
}
...
is_running=0
/etc/init.d/omnihive running >/dev/null 2>&1 && is_running=1

enabled="$(uci_get enabled '0')"
host="$(uci_get host '0.0.0.0')"
port="$(uci_get port '7575')"
data_path="$(uci_get data_path '/etc/omnihive/data')"
...
if [ -x "$BIN" ]; then
	...
	if [ "$is_running" = "1" ]; then
		_now="$(date +%s)"
		_last_query=0
		[ -f /tmp/omnihive/version_api_ts ] && _last_query="$(cat /tmp/omnihive/version_api_ts 2>/dev/null || echo 0)"
		_elapsed=$((_now - _last_query))
		if [ "$_elapsed" -ge 60 ]; then
			printf '%s' "$_now" > /tmp/omnihive/version_api_ts 2>/dev/null || true
...
if [ "$is_running" = "1" ]; then
	...
	_dc_now="$(date +%s)"
	_dc_last=0
	[ -f /tmp/omnihive/token_ts ] && _dc_last="$(cat /tmp/omnihive/token_ts 2>/dev/null || echo 0)"
	_dc_elapsed=$((_dc_now - _dc_last))

	# Refresh token at most every 30 seconds
	if [ "$_dc_elapsed" -ge 30 ] || [ ! -f /tmp/omnihive/token_cache ]; then
		printf '%s' "$_dc_now" > /tmp/omnihive/token_ts 2>/dev/null || true
		_dc_token="$(curl -s --connect-timeout 3 "http://127.0.0.1:${_dc_port}/api/auth/login" \
			-X POST -H 'Content-Type: application/json' \
			-d '{"username":"'"$(uci_get username 'admin')"'","password":"'"$(uci_get password 'admin')"'"}' \
			2>/dev/null | jsonfilter -e '@.token' 2>/dev/null || true)"
		[ -n "$_dc_token" ] && printf '%s' "$_dc_token" > /tmp/omnihive/token_cache 2>/dev/null || true
	else
		_dc_token="$(cat /tmp/omnihive/token_cache 2>/dev/null || true)"
	fi
...
plugin_version="$(pkg_installed_version luci-app-omnihive 2>/dev/null || cat /usr/share/omnihive/plugin_version 2>/dev/null || true)"
