...
(function() {
	...
	style.textContent = [
		'@keyframes omnihive-indeterminate { 0% { background-position: 200% 0; } 100% { background-position: -200% 0; } }',
...
function taskTitle(type) {
	switch (type) {
	case 'install_core':
		return _('在线安装 OmniHive 核心');
	case 'upload_core':
		return _('上传安装 OmniHive 核心');
	case 'update_plugin':
		return _('更新 LuCI 插件');
...
function taskIndeterminateBar() {
	return E('div', {
		...
		'style': [
			...
			'animation:omnihive-indeterminate 1.2s ease-in-out infinite'
		].join(';')
	}));
}
...
return view.extend({
	...
	handleSaveApply: function(ev, mode) {
		return this.super('handleSaveApply', [ ev, mode ]).then(function() {
			return fs.exec_direct('/usr/share/omnihive/apply_config.sh', []).then(notifyResult);
		});
	},
	...
	load: function() {
		return Promise.all([
			uci.load('omnihive'),
			fs.exec_direct('/usr/share/omnihive/status.sh', []).catch(function() { return '{}'; }),
			fs.exec_direct('/usr/share/omnihive/logs.sh', [ '100' ]).catch(function() { return ''; }),
			fs.exec_direct('/usr/share/omnihive/releases.sh', [ '5' ]).catch(function() { return '{"ok":false,"versions":[]}'; }),
			fs.exec_direct('/usr/share/omnihive/plugin_status.sh', [ '5' ]).catch(function() { return '{"ok":false}'; }),
			fs.exec_direct('/usr/share/omnihive/device_probe.sh', [ 'status' ]).catch(function() { return '{"ok":false}'; })
		]);
	},

	startTask: function(type, args) {
		return fs.exec_direct('/usr/share/omnihive/task_start.sh', [ type ].concat(args || []))
			.then(function(text) {
				var result = parseJson(text);
	...
	restoreRunningTask: function() {
		return fs.exec_direct('/usr/share/omnihive/task_status.sh', [])
			.then(function(text) {
	...
	pollTaskStatus: function() {
		...
		return fs.exec_direct('/usr/share/omnihive/task_status.sh', [ this.activeTaskId ])
			.then(function(text) {
				var status = parseJson(text);
				if (status.ok === false) {
	...
	cancelTask: function() {
		...
		return fs.exec_direct('/usr/share/omnihive/task_cancel.sh', [ this.activeTaskId ])
			.catch(function(e) {
				ui.addNotification(null, E('p', {}, e.message || String(e)), 'danger');
			});
	},
	...
	showUploadCoreDialog: function() {
		...
		ui.showModal(_('本地安装'), [
			E('div', { 'style': 'width:100%; box-sizing:border-box;' }, [
				E('p', { 'style': 'margin-bottom:1em;' }, _('选择本地的 OmniHive 核心二进制文件进行上传安装。')),
	...
	uploadCoreFile: function(file) {
		var self = this;
		var xhr;
		var uploadPath = '/tmp/omnihive/download/omnihive-core-upload';
	...
	renderCoreManagement: function(status, releases) {
		var m = new form.Map('omnihive');
		var s, o;
		var repoOk = releases.ok !== false && releases.latest;

		s = m.section(form.NamedSection, 'main', 'omnihive', _('核心管理'));
		s.addremove = false;
		...
		o.cfgvalue = function(section_id) {
			var value = uci.get('omnihive', section_id, 'core_arch');
		...
		o.onclick = ui.createHandlerFn(this, function() {
			...
			return m.save().then(function() {
				var version = uci.get('omnihive', 'main', 'version') || 'latest';
				var repo = uci.get('omnihive', 'main', 'release_repo') || DEFAULT_CORE_REPO;
				var arch = uci.get('omnihive', 'main', 'core_arch') || '';
			...
			depRows = deps.map(function(dep) {
				...
				if (dep.installed) {
					...
					status = E('button', {
						'class': 'btn cbi-button cbi-button-action',
						'click': ui.createHandlerFn(this, function() {
							return runScript('/usr/share/omnihive/device_tools.sh', [ dep.install_action ]);
						})
					}, _('安装依赖'));
				}
	...
	fetchDeviceProbeCache: function(devicePane, result) {
		return fs.exec_direct('/usr/share/omnihive/device_probe.sh', [ 'cache' ])
			.catch(function(e) {
	...
	pollDeviceProbe: function(devicePane, id, started, result) {
		return fs.exec_direct('/usr/share/omnihive/task_status.sh', [ id ])
			.then(function(text) {
				var status = parseJson(text);
	...
	loadDevicePane: function(devicePane, result) {
		...
		return fs.exec_direct('/usr/share/omnihive/task_start.sh', [ 'probe_device' ])
			.catch(function(e) {
	...
	runDeviceTool: function(devicePane, args, confirmText) {
		...
		return fs.exec_direct('/usr/share/omnihive/device_tools.sh', args)
			.catch(function(e) {
				return JSON.stringify({ ok: false, message: e.message || String(e) });
			})
			.then(function(text) {
	...
	renderIdentityToggle: function(devicePane, port) {
		...
		return E('div', { 'style': 'display:flex; gap:.5em; align-items:center; flex-wrap:wrap;' }, [
			...
			E('button', {
				...
				'click': ui.createHandlerFn(this, function() {
					if (!window.confirm(_('确认要将 %s 的身份切换为 %s 吗？\n\n此操作会写入模块内部 USB 配置，并重启模块。执行前会停止 OmniHive，完成后会重新启动 OmniHive。').format(port.port, targetLabel)))
						return Promise.resolve();
	...
	renderUsbnetSelect: function(devicePane, port) {
		...
		select.addEventListener('change', ui.createHandlerFn(this, function(ev) {
			...
			if (!label)
				return Promise.resolve();
			if (!window.confirm(_('确认要将 %s 的 USB 网络模式切换为 %s 吗？\n\n此操作会写入模块内部配置，并重启模块。执行前会停止 OmniHive，完成后会重新启动 OmniHive。').format(port.port, label[2])))
				return Promise.resolve();
	...
	loadNetworkStatus: function() {
		return fs.exec_direct('/usr/share/omnihive/network_status.sh', [])
			.catch(function(e) {
				return JSON.stringify({ ok: false, message: e.message || String(e) });
			})
			.then(function(text) {
	...
	renderNetworkContent: function(status) {
		...
		var omnihiveRunning = status.omnihive_running;
		...
		connRows.push(E('tr', {}, [
			...
			E('td', {}, E('span', {
				'style': 'color:%s; font-weight:700;'.format(omnihiveRunning ? '#37a24d' : '#d9534f')
			}, omnihiveRunning ? _('运行中') : _('未运行')))
		]));
		...
		integRows.push(E('tr', {}, [
			...
			}, netConfigured ? _('已创建 (%s)').format(status.netifd_device || 'omnihive') : _('未创建')))
		]));
		...
		// OmniHive network: enable or disable based on connection state
	...
	networkAction: function(action, extraArgs) {
		var self = this;
		var script = '/usr/share/omnihive/network_setup.sh';
	...
	refreshDriverPane: function(driverPane, result) {
		return fs.exec_direct('/usr/share/omnihive/usb_status.sh', [])
			.catch(function(e) {
	...
	runDriverBind: function(driverPane, args) {
		...
		return fs.exec_direct('/usr/share/omnihive/driver_bind.sh', args)
			.catch(function(e) {
				return JSON.stringify({ ok: false, message: e.message || String(e) });
			})
			.then(function(text) {
	...
	renderConfigMap: function() {
		...
		s = m.section(form.NamedSection, 'main', 'omnihive', _('基础配置'));
		s.addremove = false;
		...
		o.onclick = ui.createHandlerFn(this, function() {
			return saveApplyThen(m, function() {
				return runScript('/usr/share/omnihive/apply_config.sh', []);
			});
		});
	...
	renderStatus: function(status, releases, plugin) {
		...
		var releaseRepo = releaseRepoSlug(uci.get('omnihive', 'main', 'release_repo'));
		...
		if (!status.core_installed)
			warnings.push(E('div', { 'class': 'alert-message warning', 'style': 'margin-top:.5em;' }, _('OmniHive 核心尚未安装。')));
		...
		return E('div', { 'class': 'cbi-section' }, [
			...
			E('div', { 'style': 'display:flex; align-items:center; justify-content:space-between; margin-top:.75em; flex-wrap:wrap; gap:1em;' }, [
				...
				status.running ? E('a', { 'class': 'btn cbi-button cbi-button-action', 'target': '_blank', 'href': webUrl }, _('打开 OmniHive Web UI')) : ''
			])
		]));
	},
...
updateStatusNode: function(status) {
	...
	refreshStatus: function() {
		return fs.exec_direct('/usr/share/omnihive/status.sh', [])
	...
	renderServiceButtons: function() {
		return E('span', {}, [
			E('button', {
				'class': 'btn cbi-button cbi-button-apply',
				'click': ui.createHandlerFn(this, function() { return runScript('/usr/share/omnihive/service.sh', [ 'start' ]); })
			}, _('启动')),
			' ',
			E('button', {
				'class': 'btn cbi-button cbi-button-reset',
				'click': ui.createHandlerFn(this, function() { return runScript('/usr/share/omnihive/service.sh', [ 'stop' ]); })
			}, _('停止')),
			' ',
			E('button', {
				'class': 'btn cbi-button cbi-button-reload',
				'click': ui.createHandlerFn(this, function() { return runScript('/usr/share/omnihive/service.sh', [ 'restart' ]); })
			}, _('重启')),
			' ',
			E('button', {
				'class': 'btn cbi-button cbi-button-reset',
				...
				'click': ui.createHandlerFn(this, function() {
					if (!window.confirm(_('将重置模组并恢复 QMI 通信，期间服务会短暂中断，是否继续？')))
						return Promise.resolve();
					return runScript('/usr/share/omnihive/recover_qmi.sh', []);
				})
			}, _('修复'))
		]);
	},
	...
	refreshLogs: function(logNode) {
		return fs.exec_direct('/usr/share/omnihive/logs.sh', [ '100' ])
			.catch(function() { return ''; })
			.then(function(logs) {
	...
	clearLogs: function(logNode) {
		return fs.exec_direct('/usr/share/omnihive/clear_logs.sh', [])
			.then(function(text) {
				notifyResult(text);
	...
	render: function(data) {
		...
		if (!releases.repo)
			releases.repo = releaseRepoSlug(uci.get('omnihive', 'main', 'release_repo'));
			...
			return this.renderOverview(status, releases, plugin, deviceDeps).then(function(overviewEl) {
				...
				return E('div', {}, [
					E('div', { 'class': 'cbi-map-descr' }, [
						_('OmniHive 的 OpenWrt 管理插件，在路由器界面中完成核心安装、服务控制、配置管理与 USB 驱动运维。'),
						E('br'),
						_('仓库地址：'),
